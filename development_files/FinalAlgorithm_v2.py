    ...
    def BinCategoricalFeatures(tenK_df4):
        ...
        for n,i in enumerate(tenK_df4.dtypes):
            ...
            if i[1]=='string':
                ...
                ## if count of "label = 1" is zero for a category, its value will be null

                df1 = click_freqDF.alias('df1')
                if n == 0:
                    df2 = tenK_df4.alias('df2')
                else:
                    df2 = tenK_click_df.alias('df2')

                tenK_click_df = df2.join(df1, [feature], "left_outer").select('df2.*',
                                        df1['count'].alias(feature+"_click"))
                ...
                F.when(tenK_click_df[feature+'_click'].isNotNull(), F.lit("B"))
                .otherwise(F.lit("A")))

                tenK_click_df = tenK_click_df.drop(feature+"_click")
...
top_features = ['OHE__38_B', 'OHE__25_A', 'OHE__29_A', 'OHE__38_A', '_7_imputed', 'OHE__29_B',
                'OHE__25_B', 'OHE__27_A', '_12_imputed', '_14_imputed', 'OHE__32_A', '_8_imputed',
                '_9_imputed', '_6_imputed', 'OHE__27_B', 'OHE__16_B', 'OHE__15_A', '_4_imputed',
                '_10_imputed', '_3_imputed', 'OHE__22_A', 'OHE__16_A', '_5_imputed', 'OHE__32_B',
                'OHE__15_B', 'OHE__20_A', 'OHE__20_B', 'OHE__19_A', 'OHE__19_B', 'OHE__31_B']
